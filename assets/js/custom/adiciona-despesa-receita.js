...
class SaveAccounts {
  constructor(getValueModified, $getName) {
    (this.nome = $getName), (this.valor = Math.abs(getValueModified));
  ...
  insertAccount() {
    ...
    conta.insertAdjacentText(
      "afterbegin",
      `${this.nome} R$ ${this.checkerNumberFormatting()} ${this.valor}`
    );
...
function calcAccount(accounts) {
  ...
  if (calcfaturamento > 0) {
    $faturamento.textContent = `R$ + ${calcfaturamento.toLocaleString(
      "pt-BR"
    )}`;
  } else if (calcfaturamento < 0) {
    $faturamento.textContent = `R$ - ${Math.abs(calcfaturamento).toLocaleString(
      "pt-BR"
    )}`;
  } else {
  ...
  if (accountsDebitos.length != 0) {
    $boxDebitos.textContent = `R$ - ${Math.abs(calcDebitos).toLocaleString(
      "pt-BR"
    )}`;
  } else {
